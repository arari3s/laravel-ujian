...
class UserQuizlv extends Component
{
    ...
    public function showResults()
    {
        ...
        $this->totalQuizQuestions = Quiz::where('quiz_headers_id', $this->quizid->id)->count();

        // Get a count of correctly answered questions for this quiz.
        $this->currectQuizAnswers = Quiz::where('quiz_headers_id', $this->quizid->id)
            ->where('is_correct', '1')
            ->count();
    ...
    {
        $this->question = Question::withCount('course')
            // ->join('courses', 'questions.id', '=', 'courses.id')
    ...
    public function getNextQuestion()
    {
        ...
        $question = Question::where('courses_id', $this->questionId)
            ->whereNotIn('id', $this->answeredQuestions)
            ->with('questionbank.answer')
            ->inRandomOrder()
            ->first();
